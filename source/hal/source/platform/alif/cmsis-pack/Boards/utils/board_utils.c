...
// Use separate LED instances when there are more than one leds on board
#if (BOARD_LEDRGB_COUNT > 1)
#define LED_G_PORT BOARD_LEDRGB1_G_GPIO_PORT
#define LED_G_PIN  BOARD_LEDRGB1_G_GPIO_PIN
#else
#define LED_G_PORT BOARD_LEDRGB0_G_GPIO_PORT
#define LED_G_PIN  BOARD_LEDRGB0_G_GPIO_PIN
#endif
...
extern ARM_DRIVER_GPIO ARM_Driver_GPIO_(LED_G_PORT);
...
void BOARD_UTILS_Init()
{
...
#ifdef GPIO_PROFILING
    ARM_DRIVER_GPIO *BOARD_LED1_BLUE_GPIOdrv = &ARM_Driver_GPIO_(BOARD_LEDRGB0_B_GPIO_PORT);
#endif
	ARM_DRIVER_GPIO *BOARD_LED2_GPIOdrv = &ARM_Driver_GPIO_(LED_G_PORT);
...
#ifdef GPIO_PROFILING
    BOARD_LED1_BLUE_GPIOdrv->Initialize(BOARD_LEDRGB0_B_GPIO_PIN, NULL);
    BOARD_LED1_BLUE_GPIOdrv->PowerControl(BOARD_LEDRGB0_B_GPIO_PIN, ARM_POWER_FULL);
    BOARD_LED1_BLUE_GPIOdrv->SetValue(BOARD_LEDRGB0_B_GPIO_PIN, GPIO_PIN_OUTPUT_STATE_LOW);
    BOARD_LED1_BLUE_GPIOdrv->SetDirection(BOARD_LEDRGB0_B_GPIO_PIN, GPIO_PIN_DIRECTION_OUTPUT);
#endif
    ...
    BOARD_LED2_GPIOdrv->Initialize(LED_G_PIN, NULL);
    BOARD_LED2_GPIOdrv->PowerControl(LED_G_PIN, ARM_POWER_FULL);
    BOARD_LED2_GPIOdrv->SetValue(LED_G_PIN, GPIO_PIN_OUTPUT_STATE_LOW);
    BOARD_LED2_GPIOdrv->SetDirection(LED_G_PIN, GPIO_PIN_DIRECTION_OUTPUT);
}
...
{
	ARM_DRIVER_GPIO *BOARD_LED2_GPIOdrv = &ARM_Driver_GPIO_(LED_G_PORT);
	BOARD_LED2_GPIOdrv->SetValue(LED_G_PIN, (GPIO_PIN_OUTPUT_STATE) state);
}
...
{
    ARM_DRIVER_GPIO *BOARD_LED1_BLUE_GPIOdrv = &ARM_Driver_GPIO_(BOARD_LEDRGB0_B_GPIO_PORT);
    BOARD_LED1_BLUE_GPIOdrv->SetValue(BOARD_LEDRGB0_B_GPIO_PIN, (GPIO_PIN_OUTPUT_STATE) state);
}
#endif
