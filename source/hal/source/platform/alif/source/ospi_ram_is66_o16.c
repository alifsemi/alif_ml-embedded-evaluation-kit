...
#define REG_CONFIG              0x00080000

#define RAM_DEVICE_DEFAULT_LC                   7
...
static void ospi_ram_enable_xip(void)
{
    ...
    val = (SPI_FRAME_FORMAT_OCTAL << XIP_CTRL_FRF_OFFSET)
            | (SPI_TRANS_TYPE_DUAL_OCTAL << XIP_CTRL_TRANS_TYPE_OFFSET)
    ...
    val = (SPI_FRAME_FORMAT_OCTAL << XIP_WRITE_CTRL_WR_FRF_OFFSET)
             | (SPI_TRANS_TYPE_DUAL_OCTAL << XIP_WRITE_CTRL_WR_TRANS_TYPE_OFFSET)
    ...
    //
    //  --   RA7  CA9  --                         --  18    10  --
    // RA14  RA6  CA8  --                         25  17     9  --
    // RA13  RA5  CA7  --                         24  16     8  --
    // RA12  RA4  CA6  --                         23  15     7  --
    // RA11  RA3  CA5  CA3                        22  14     6   4
    // RA10  RA2  CA4  CA2                        21  13     5   3
    //  RA9  RA1  --   CA1                        20  12    --   2
    //  RA8  RA0  --   CA0 (=0)                   19  11    --   1 (=0)
    //
    // SPI controller shifts HADDR down 1 when in dual octal mode to form a 16-bit word address.
    // We massage HADDR using the shim in front of the controller to insert the gap between CA3 and CA4 - between HADDR[4] and HADDR[5]

    aes->AES_ADDR_CONTROL = (5 << 28) // 5 bits in the lower part of the address
                          | (11 << 22) // next bit HADDR[5] needs to be shifted to HADDR[11] to end up as bit 10 of the SPI address
                          | (3 << 18) // apply transformation to both slave selects
                          | (0x001F << 0); // mask for lower 5 bits
...
static int32_t reg_read(uint32_t reg, uint16_t *value)
{
    ...
    ret = ptrOSPI->Control(ARM_OSPI_SET_ADDR_LENGTH_WAIT_CYCLE | control_flags, (ARM_OSPI_ADDR_LENGTH_32_BITS << ARM_OSPI_ADDR_LENGTH_POS) | (current_wait_cycles << ARM_OSPI_WAIT_CYCLE_POS));
    if (ret != ARM_DRIVER_OK)
    {
...
int32_t ospi_ram_init(void)
{
    ...
    /* Note that register reads and writes are 8-bit; only XIP uses dual octal */
    ...
    printf("Attempting IS66_O16 RAM initialisation\n");
    ...
    printf("HSPI RAM apparently okay\n");
    ...
    *(volatile uint32_t *) 0xC0000000 = 0x12345678;
    ...
    (void)*(volatile uint32_t *) 0xC0000000;
    printf("XIP test HSPI RAM OK\n");

    printf("Linear test HSPI RAM\n");
    ram_linear_test((uint8_t *) 0xC0000000);

    printf("Random test HSPI RAM\n");
    ram_random_test((uint8_t *) 0xC2000000);
