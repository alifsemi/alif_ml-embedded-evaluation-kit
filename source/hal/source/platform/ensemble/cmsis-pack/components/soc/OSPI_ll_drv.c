...
void OSPI_ll_IRQHandler(OSPI_RESOURCES *OSPI)
{
    ...
    uint32_t event, val, tx_data, index;
    uint16_t tx_count, rx_count, frame_size;
    ...
    if (event & SPI_TX_FIFO_EMPTY_EVENT)
    {
        ...
        reg_ptr->txftlr &= ~(0xff << SPI_TXFTLR_TXFTHR_POS);
        reg_ptr->txftlr |= ((tx_count - 1) << SPI_TXFTLR_TXFTHR_POS);
    ...
    if (event & SPI_RX_FIFO_FULL_EVENT)
    {
        ...
        else
        {
            for (index = 0; index < rx_count; index++)
            {
                OSPI->rx_buff[OSPI->rx_current_cnt] = (uint8_t) (reg_ptr->data_reg);
...
int32_t OSPI_ll_Transfer(OSPI_RESOURCES *OSPI, const void *data_out, void *data_in, uint32_t num)
{
    ...
    OSPI->tx_buff = (uint32_t *) data_out;
    OSPI->rx_buff = (uint8_t *) data_in;
