...
extern ARM_DRIVER_GPIO Driver_GPIO1;
ARM_DRIVER_GPIO *GPIO_Driver_BLED = &Driver_GPIO1;
#elif (RTE_ILI9806E_PANEL_BL_LED_GPIO_PORT == 2)
extern ARM_DRIVER_GPIO Driver_GPIO2;
ARM_DRIVER_GPIO *GPIO_Driver_BLED = &Driver_GPIO2;
#elif (RTE_ILI9806E_PANEL_BL_LED_GPIO_PORT == 3)
extern ARM_DRIVER_GPIO Driver_GPIO3;
ARM_DRIVER_GPIO *GPIO_Driver_BLED = &Driver_GPIO3;
#elif (RTE_ILI9806E_PANEL_BL_LED_GPIO_PORT == 4)
extern ARM_DRIVER_GPIO Driver_GPIO4;
ARM_DRIVER_GPIO *GPIO_Driver_BLED = &Driver_GPIO4;
#else
...
/*Commands for Page 7*/
LCD_Page_cmd page7_cfg[] =
{
	{0x18, 0x1D}, {0x02, 0x77}, {0xE1, 0x79}
};

/*Commands for Page 0*/
LCD_Page_cmd page0_cfg[] =
{
	{0x36, 0x01},
	{0x3A, 0x70}
};
...
static void ILI9806E_Configure(void)
{
	...
	for(int i = 0; i < ARRAY_SIZE(page7_cfg); i++)
	...
	for(int i = 0; i < ARRAY_SIZE(page5_cfg); i++)
	...
	for(int i = 0; i < ARRAY_SIZE(page0_cfg); i++)
...
/**
  ...
  \return       \ref execution_status
  */
int32_t ILI9806E_Control(uint32_t control)
{
	switch(control)
	{
...
DISPLAY_PANEL_OPERATIONS ILI9806E_display_ops =
{
	.Init    = ILI9806E_Init,
...
DISPLAY_PANEL_FRAME_INFO ILI9806E_display_frame_info =
{
...
DISPLAY_PANEL_INFO ILI9806E_panel_info =
{
